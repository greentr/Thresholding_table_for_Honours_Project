...
if args.truthset:
    ...
    quit()
truthsetpysam = VariantFile(truth_set)
truthsetnumber = 0
for rec in truthsetpysam.fetch():
    truthsetnumber += 1
...
f = open("Output_%s_%s.tsv" % (condition, now.strftime("%Y-%m-%d %H:%M")), "w")
f.write("Proportion \t")        
f.write("Depth \t")
f.write("Sensitivity \t")
f.write("PPV \n")
...
for pct in pct_options:
    for np in np_options:
        ...
        os.chdir(current_dir)
        f = open("Output_%s_%s.tsv" % (condition, now.strftime("%Y-%m-%d %H:%M")), "a")
        f.write("%s \t" % pct)        
        f.write("%s \t" % np)
        f.write("%s \t" % x)
        f.write("%s \n" % y)
